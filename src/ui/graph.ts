import * as d3 from 'd3';
import type { DatasetMatch } from '../types';
import { truncate } from './dom';

interface LandscapeNode extends d3.SimulationNodeDatum {
  match: DatasetMatch;
  topic: string;
}

const MAX_NODES = 80;
const MAX_LABELS = 24;

/**
 * Render a deterministic, pre-settled semantic landscape.
 *
 * The previous version exposed a live force simulation to the user. That made
 * the catalogue look lively but poor to work with: selecting a dataset or
 * resizing the window restarted visible motion. We now seed positions, tick
 * the simulation off-screen, stop it, and render only the settled coordinates.
 */
export function renderGraph(
  container: HTMLElement,
  svgSelection: d3.Selection<SVGSVGElement, unknown, HTMLElement, unknown>,
  matches: DatasetMatch[],
  selectedId: string | null,
  onSelect: (id: string) => void,
): void {
  const rect = container.getBoundingClientRect();
  const width = Math.max(480, rect.width);
  const height = Math.max(360, rect.height);
  svgSelection.attr('viewBox', `0 0 ${width} ${height}`);
  svgSelection.selectAll('*').remove();

  const data = matches.slice(0, MAX_NODES);
  if (!data.length) {
    svgSelection.append('text').attr('x', 24).attr('y', 42).attr('class', 'empty-note').text('No datasets match this filter.');
    return;
  }

  const nodes: LandscapeNode[] = data.map((match, index) => {
    const angle = seededAngle(match.dataset.id, index);
    const seedRadius = 18 + (index % 11) * 8;
    return {
      match,
      topic: match.dataset.semantic.topics[0] ?? match.dataset.themes[0]?.toLowerCase() ?? 'other',
      x: width / 2 + Math.cos(angle) * seedRadius,
      y: height / 2 + Math.sin(angle) * seedRadius,
    };
  });

  const topics = [...new Set(nodes.map(node => node.topic))];
  const centers = new Map(
    topics.map((topic, index) => {
      const angle = (index / Math.max(1, topics.length)) * Math.PI * 2 - Math.PI / 2;
      return [topic, { x: width / 2 + Math.cos(angle) * width * 0.24, y: height / 2 + Math.sin(angle) * height * 0.24 }];
    }),
  );

  const radiusFor = (node: LandscapeNode) => 9 + Math.max(4, node.match.relevance.score * 0.13);
  const labelCutoff = [...nodes]
    .sort((a, b) => b.match.relevance.score - a.match.relevance.score)
    .slice(0, MAX_LABELS)
    .at(-1)?.match.relevance.score ?? 0;
  const isLabelled = (node: LandscapeNode) =>
    node.match.relevance.score >= labelCutoff || selectedId === node.match.dataset.id;

  const simulation = d3
    .forceSimulation(nodes)
    .force('x', d3.forceX<LandscapeNode>(d => centers.get(d.topic)?.x ?? width / 2).strength(0.2))
    .force('y', d3.forceY<LandscapeNode>(d => centers.get(d.topic)?.y ?? height / 2).strength(0.2))
    .force('charge', d3.forceManyBody().strength(-48))
    .force('collide', d3.forceCollide<LandscapeNode>(d => radiusFor(d) + (isLabelled(d) ? 40 : 12)))
    .stop();

  for (let i = 0; i < 180; i += 1) simulation.tick();

  const group = svgSelection.append('g');
  const node = group
    .selectAll<SVGGElement, LandscapeNode>('g')
    .data(nodes)
    .enter()
    .append('g')
    .attr('class', d => `node ${selectedId === d.match.dataset.id ? 'selected' : ''}`)
    .attr('transform', d => `translate(${clamp(d.x ?? 0, 40, width - 40)},${clamp(d.y ?? 0, 30, height - 46)})`)
    .style('cursor', 'pointer')
    .on('click', (_, d) => onSelect(d.match.dataset.id));

  node.append('title').text(d => `${d.match.dataset.title} · relevance ${d.match.relevance.score}`);
  node.append('circle').attr('r', radiusFor).attr('fill', d => fill(d.match));

  const labelled = node.filter(isLabelled);
  labelled
    .append('text')
    .attr('class', 'node-label')
    .attr('y', d => radiusFor(d) + 13)
    .text(d => truncate(d.match.dataset.title, 24));
  labelled
    .append('text')
    .attr('class', 'node-sub')
    .attr('y', d => radiusFor(d) + 23)
    .text(d => `${d.match.relevance.score} · ${d.match.dataset.id}`);
}

/** Kept for the existing stage transition API; layouts are now already stopped. */
export function stopGraph(): void {}

const clamp = (value: number, min: number, max: number): number => Math.max(min, Math.min(max, value));

function seededAngle(id: string, index: number): number {
  let hash = 2166136261;
  for (const char of id) hash = Math.imul(hash ^ char.charCodeAt(0), 16777619);
  return (((hash >>> 0) % 10000) / 10000 + index * 0.61803398875) * Math.PI * 2;
}

function fill(match: DatasetMatch): string {
  switch (match.evidenceClass) {
    case 'direct':
      return '#2d5b49';
    case 'supporting':
      return '#7fa694';
    default:
      return match.relevance.score >= 30 ? '#c0c8c2' : '#dcdad3';
  }
}
